---
name: git-commit
description: Use when the user asks to commit changes, commit selected files, the current file, or the current folder and wants an English Conventional Commit message drafted for approval before committing.
---
...
- `scope` 是可选项，放在圆括号中，例如：`docs(codex):`
- `description` 是必填项，紧跟在 `type/scope` 后面
- `body` 是可选项；如果存在，必须与标题之间空一行
- 在本项目语境中，Conventional Commits 的 `body` 由 `description` 承载
...
- 结合本项目约定：
  ...
  - 默认使用小写开头的祈使或说明式短句
  - 标题末尾不加句号
  - 如果改动围绕单一简单任务，且标题已足够表达意图，可以省略 `description`
  - 如果一次提交包含多个任务，或一个任务中还包含多个子任务，应通过 `description` 做清晰说明
  - 如果 `description` 同时覆盖多个不同任务，使用一个英文列表逐项描述，而不是拆成多个独立段落
...
- 如果 `Proposed description` 包含多个不同任务，使用一个列表展示这些任务项
- `Please confirm:` 请用户确认是否执行提交，并可接受简短确认词如 `y`、`ok`、`go`、`提交`、`好的`、`好`、`确认`
